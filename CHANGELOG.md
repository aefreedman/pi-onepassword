...
- Added `scripts/start-pi-codecks-external-helper.ps1`, a repository-only trusted local launcher for a fresh normal Pi Codecks session. It takes no configuration arguments; it uses masked prompts, resolves this source adapter plus trusted local `op` and `pi` applications, sanitizes and restores only the process environment, does not acknowledge live validation, and returns Pi's exit code. It is intentionally excluded from package archives; deterministic fake-application coverage verifies the child environment, quote normalization, redaction, exit propagation, and restoration without a network or credentials.

...
- Added repository-only TypeScript and PowerShell live Codecks read-only authentication launchers with a fixed sibling child path, masked fallback prompts, safe reuse of existing environment configuration, process-scope restoration, redacted authentication/reference/consumer stages, and deterministic no-network wiring/non-disclosure coverage. They are excluded from the published tarball.

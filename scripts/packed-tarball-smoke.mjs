...
try {
  ...
  for (const name of Object.keys(cleanEnv)) if (/^(?:OP_|PI_ONEPASSWORD_)/i.test(name)) delete cleanEnv[name];
  ...
  for (const excludedPath of ["tests", "scripts", ".github", "package-lock.json", "tsconfig.json", "extensions/integrations"]) {
...
assert.deepEqual(env.sanitizeOnePasswordEnvironment({ PATH: "/safe", Op_Service_Account_Token: "inert", OP_SESSION_WORK: "ambient" }), { PATH: "/safe" });
